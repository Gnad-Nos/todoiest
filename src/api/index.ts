...
export class TodoistApiClient {
  ...
  public async getTasks(filter?: string): Promise<Task[]> {
    ...
    console.log(camelize(JSON.parse(response.body)) as Task[])
  ...
  public async getCompleted(): Promise<Task[]> {
    const response = await this.do("/get_all", "GET");
    return camelize(JSON.parse(response.body)) as Task[];
  }
  ...
  private async do1(path: string, method: string, json?: object): Promise<WebResponse> {
    const params: RequestParams = {
      url: `https://api.todoist.com/sync/v9/completed`,
      method: method,
      headers: {
        "Authorization": `Bearer ${this.token}`,
      },
